...
#define TDIN1 HAL_GPIO_WritePin (T_DIN_GPIO_Port,T_DIN_Pin,GPIO_PIN_SET)
#define TDIN0 HAL_GPIO_WritePin (T_DIN_GPIO_Port,T_DIN_Pin,GPIO_PIN_RESET)

#define TCLK1 HAL_GPIO_WritePin (T_CLK_GPIO_Port,T_CLK_Pin,GPIO_PIN_SET)
#define TCLK0 HAL_GPIO_WritePin (T_CLK_GPIO_Port,T_CLK_Pin,GPIO_PIN_RESET)

#define TCS1 HAL_GPIO_WritePin (T_CS_GPIO_Port,T_CS_Pin,GPIO_PIN_SET)
#define TCS0 HAL_GPIO_WritePin (T_CS_GPIO_Port,T_CS_Pin,GPIO_PIN_RESET)

#define DOUT HAL_GPIO_ReadPin(T_DO_GPIO_Port, T_DO_Pin)

void delayUs(unsigned int Delay)
{
	__HAL_TIM_SET_COUNTER(&time_us, 0);  // reset the counter
	while ((__HAL_TIM_GET_COUNTER(&time_us))<Delay);
}
...
{ 	 
	...
	{ 				  
		...
		delayUs(20);    
 		TCLK1;
 		if(HAL_GPIO_ReadPin (T_DO_GPIO_Port , T_DO_Pin))
                {
                  Num++;
                }
	}  	
