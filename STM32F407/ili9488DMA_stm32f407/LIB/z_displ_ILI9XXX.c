...
//
//void Displ_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t* data){
//#ifdef Z_RGB565
...
void drawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h,const uint8_t* img )
{
	...
	HAL_GPIO_WritePin(DISPL_CS_GPIO_Port, DISPL_CS_Pin, GPIO_PIN_RESET);
        ...
        uint8_t* linebuff = (uint8_t*)malloc(w * 3 + 1);
        
        if(linebuff == NULL) show_uart("Error");
	uint32_t count = 0;
	for (uint16_t i = 0; i < h; i++) {
		...
		for (uint16_t o = 0; o < w; o++) {
			...
			pixcount++;
                 ...
                 HAL_SPI_Transmit(&DISPL_SPI_PORT , linebuff, w * 3, HAL_MAX_DELAY);
	}
