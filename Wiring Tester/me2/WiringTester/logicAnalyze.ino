...
void logicAnalyze() {
  ...
  //********************+12v*********************
  if (InputVoltage >= 11 && ADCres >= 2500) {
  ...
  if (ADCres > 1470) {  //
    resistant *= 1.06;
    resistant *= 1.3;
    if (ADCres >= 1460 && ADCres < 1500) resistant *= 1.8333333333;
  ...
  if (ADCres <= 1470) {
    resistant = map(ADCres, 1420, 1470, 0, 100);
  ...
  //********************Sensor*********************
  if (InputVoltage >= .1 && InputVoltage < 11 && ADCres >= 2500) {  // sensor
    if (mute == 0) {
      digitalWrite(buzzer, !digitalRead(buzzer));
      delay(InputVoltage);
      digitalWrite(buzzer, !digitalRead(buzzer));
  ...
  //******vlotmeter******
  tft.setTextSize(2);
  text = "ADC:" + String(ADCres, 1);
  tft.setTextColor(ILI9488_WHITE);
  tft.fillRect(105, 140, 190, 20, ILI9488_BLACK);
